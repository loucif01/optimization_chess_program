...
/*********************************************************************/
/***							      	   ***/
/***         	TP MinMax / Module TPGO / ESI 2018-2019            ***/
/***							      	   ***/
/*** Il est demandé d'améliorer une des (05) fonctions d'éstimation données. ***/
/*** Nous avons choisis la 1ere fonction estim1(...) et l'amiliorer par une autre fonction estim(...)***/
...
// evaluation table by position
const int piece_square_table [5][64] = {
{
// Pawns - early/mid
//  A   B   C   D   E   F   G   H
    0,  0,  0,  0,  0,  0,  0,  0, // 1
...
// ********************************************************
	...
	for (i=0; i<8; i++)
	   ...
	   for (j=0; j<8; j++) {
		...
		switch (conf.mat[i][j]) {
		   ...
		   case -'p' : pionN++;
                ScrQte -= piece_square_table[0][mirror[i*8+j]];
                break;
		   case -'c' : cN++;
                ScrQte -= piece_square_table[1][mirror[i*8+j]];
                break;
		   case -'f' : fN++;
                ScrQte -= piece_square_table[2][mirror[i*8+j]];
                break;
		   case -'t' : tN++; 
                ScrQte -= piece_square_table[3][mirror[i*8+j]];
                break;
		   case -'n' : nN++;
                ScrQte -= piece_square_table[4][mirror[i*8+j]];
                break;
		}
	   }
	...
	ScrQte += ( (pionB*100 + cB*320 + fB*330 + tB*500 + nB*900) - (pionN*100 + cN*320 + fN*330 + tN*500 + nN*900) )* 100.0/76;;

	while (ScrQte > 95) ScrQte = ScrQte / 2;		// pour rétrécir l'intervalle à
    
	while (ScrQte < -95) ScrQte = ScrQte / 2;		// ]-95 , +95[ car ce n'est qu'une estimation
